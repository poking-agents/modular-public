...
def validate_middleman_settings(
    middleman_settings: MiddlemanSettings | None,
) -> MiddlemanSettings:
    if middleman_settings is None:
        raise ValueError(
            "Do not call this function directly. Use a partial application of it instead."
        )
    return middleman_settings


async def _claude_legacy_factory(
    agent: Agent, middleman_settings: MiddlemanSettings | None = None
) -> None:
    middleman_settings = validate_middleman_settings(middleman_settings)
...
async def _program_synthesis_factory(
    agent: Agent, middleman_settings: MiddlemanSettings | None = None
) -> None:
    middleman_settings = copy.deepcopy(validate_middleman_settings(middleman_settings))
    middleman_settings.stop = [f"</{tool}" for tool in agent.toolkit_dict]
    ...
    match agent.state.next_step["args"]["mode"]:
        case "revision":
            ...
            assert generations.outputs is not None
            output = generations.outputs[0]
            generation = output.completion
            open_tag_locs = {
                tool: generation.find(f"<{tool}>") for tool in agent.toolkit_dict
            }
        ...
        case "synthesis":
                ...
                for output in generations.outputs:
                    ...
                    if open_tag_locs[last_tool] != -1:
                        ...
                        function_call = {
                            "type": "function",
                            "name": last_tool,
                            "arguments": raw_function_call.removesuffix(
                                f"</{last_tool}>"
                            ),
                        }
                    message = Message(
            ...
            agent.state.next_step["module_type"] = "discriminator"
            agent.state.next_step["args"]["options"] = generated_messages
...
for model_pair, n in product(claude_legacy_compat_models, [1, 2, 4, 8, 16, 32, 64]):
    ...
    globals()[f"_program_synthesis_{n}x{desc}"] = partial(
        _program_synthesis_factory,
        middleman_settings=MiddlemanSettings(
            n=n,
            model=model,
            temp=1,
